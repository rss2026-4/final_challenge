...
from vis_msgs.msg import ObjectLocation, ParkingError
...
class ParkingController(Node):
    ...
    def state_callback(self, msg):
        if msg.current_state == State.PARKING_METER:
            self.disabled = False
        else:
            self.disabled = True
    ...
    def relative_parking_callback(self, msg):
        ...
        if self.backing_up:
            ...
            if abs(self.relative_y) > 0.3:
                ...
                # SAVE IMAGE -- change state to image save
                state = State()
                state.current_state = State.PARKED
                self.get_logger().info("Published parked state")
        ...
        #################################
        # self.get_logger().info(f"{drive_cmd.drive.speed:.2f} m/s")
