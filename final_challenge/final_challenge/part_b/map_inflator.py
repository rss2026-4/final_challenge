...
class MapInflator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("map_sub",  "/map")
        self.declare_parameter("map_pub",  "/inflated_map")

        self.MAP_SUB = self.get_parameter("map_sub").get_parameter_value().string_value
        self.MAP_PUB = self.get_parameter("map_pub").get_parameter_value().string_value

        self.map_sub = self.create_subscription(OccupancyGrid, self.MAP_SUB, self.map_cb, latched_qos)
        self.map_pub = self.create_publisher(OccupancyGrid, self.MAP_PUB, latched_qos)
