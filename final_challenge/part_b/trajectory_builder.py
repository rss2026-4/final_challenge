...
class BuildTrajectory(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("click_sub",  "/clicked_point")
        self.declare_parameter("traj_pub",  "/trajectory/current")
        self.declare_parameter("traj_pts",  "/traj_pts")
        self.CLICK_SUB = self.get_parameter("click_sub").get_parameter_value().string_value
        self.TRAJ_PUB = self.get_parameter("traj_pub").get_parameter_value().string_value
        self.TRAJ_PTS = self.get_parameter("traj_pts").get_parameter_value().string_value


        self.click_sub = self.create_subscription(PointStamped, self.CLICK_SUB, self.clicked_pose, 10)
        self.traj_pub = self.create_publisher(PoseArray, self.TRAJ_PUB, traj_qos)
        self.trajectory_points = self.create_publisher(Marker, self.TRAJ_PTS, 20)
