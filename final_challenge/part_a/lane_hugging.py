...
class LaneHugging(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            f"LaneHugging initialized with homography {homography_path}"
        )
    ...
    def detect_lanes(self, img):
        ...
        if self.last_detection.left_line is None:
            self.lookahead_uv = None
            return None

        left_line = np.asarray(self.last_detection.left_line, dtype=np.float64)
        target_line = left_line.copy()
        target_line[1] += self.pipeline_config.lane_width_px + self.pipeline_config.center_offset_px
        self.last_detection.center_line = target_line
        self.last_detection.right_line = None

        lookahead = choose_lookahead(target_line, img.shape, self.homography, self.lookahead_distance_m, self.lookahead_samples, self.lookahead_row_pct, self.last_detection.roi_y)
