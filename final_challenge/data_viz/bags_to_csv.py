#!/usr/bin/env python3
"""
Convert every ROS2 bag in `../part a bags` into CSV files — no ROS install required.

Install dependency:
    pip install rosbags

Usage:
    python3 bags_to_csv.py [--input DIR] [--output DIR] [--topics TOPIC [TOPIC ...]]

Defaults:
    --input   <script_dir>/../part a bags   (scans for subdirs containing metadata.yaml)
    --output  <script_dir>/../part a csvs
    --topics  (all topics in the bag)

One CSV is written per bag. Each row carries a timestamp, the topic name,
and one column per (topic-prefixed) message field.
"""
